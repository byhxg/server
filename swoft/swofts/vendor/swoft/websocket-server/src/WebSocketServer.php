...
class WebSocketServer extends HttpServer
{
    ...
    public function start()
    {
        ...
        //Start Tcp Server
        if ((int)$this->serverSetting['tcpables'] === 1) {
            $this->registerTcpEvent();
        }
    ...
    protected function registerTcpEvent()
    {
        $swooleListeners = SwooleListenerCollector::getCollector();
        if (!isset($swooleListeners[SwooleEvent::TYPE_TCP]) || empty($swooleListeners[SwooleEvent::TYPE_PORT])) {
            throw new RuntimeException("Please use swoft/tcp-server, run 'composer require swoft/tcp-server'");
        }
        $listen = $this->server->listen($this->tcpServerSetting['host'],$this->tcpServerSetting['port'], $this->tcpServerSetting['type']);
        $tcpSetting = $this->getListenTcpSetting();
        $listen->set($tcpSetting);
        $swooleTcpPortEvents = $swooleListeners[SwooleEvent::TYPE_TCP];
        $this->registerSwooleEvents($listen, $swooleTcpPortEvents);
    }
    /**
     * @param string $msg
     * @param array $data
