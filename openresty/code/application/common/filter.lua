...
-- 在redis中嵌入lua脚本
-- 布隆redis中直接操作
--192.168.5.12:6379> BF.add test 1
--(integer) 1
--192.168.5.12:6379> BF.add test 3
--(integer) 1
--192.168.5.12:6379> BF.EXISTS test 1
--(integer) 1

local res ,err = red_c:eval([[
    local key = KEYS[1]
    local val  = ARGV[1]
--    local redis.call('bf.add',key,val)
    local res,err =  redis.call('bf.exists',key,val)
    return res
]],1,'test',id);
-- '{test}:1',id 这种形式可以达到放到同一个节点上
